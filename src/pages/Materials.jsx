...
const Materials = () => {
  ...
  const updateAggregatedData = (data, project) => {
    const filteredData =
      project === "All"
        ? data
        : data.filter((item) => item.Project_ID === project);
    const aggregated = {};
    const alertList = [];
    ...
    filteredData.forEach((item) => {
      ...
      aggregated[item.Project_ID][item.Material] =
        (aggregated[item.Project_ID][item.Material] || 0) + item.Quantity;
    });
        ...
        <div className="alert-card">
          ...
          <div className="alert-buttons">
            {["Critical", "Medium", "High"].map((level) => (
              <button
                ...
                onClick={() => setSelectedAlertLevel(level)}
              >
                {level} (
          ...
          {selectedAlertLevel && (
            <div className="alert-details">
              {alerts
                .filter((alert) => alert.level === selectedAlertLevel)
                .map((alert, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <p>
                      <strong>{alert.material}</strong>: {alert.quantity} Near
                      to Out of Stock
                    </p>
                  </div>
                ))}
