...
const AddNewMaterial = () => {
  ...
  const [materials, setMaterials] = useState([{ materialName: "", searchTerm: "", filteredMaterials: [] }]);
  ...
  useEffect(() => {
    fetch("/data/project_all_materials_updated.xlsx")
      ...
      .then((buffer) => {
        ...
        const uniqueProjectIds = [...new Set(jsonData.map((item) => item.Project_ID))];
  ...
  const handleProjectChange = (e) => {
    ...
    if (selectedId) {
      const filteredSuppliers = [...new Set(stock.filter(item => item.Project_ID === selectedId).map(item => item.Supplier))];
  ...
  const handleSupplierChange = (e) => {
    ...
    if (supplier && suppliers.includes(supplier)) {
      const materialsForSupplier = stock.filter(item => item.Project_ID === selectedProjectId && item.Supplier === supplier);
      setAvailableMaterials(materialsForSupplier.length ? materialsForSupplier.map(item => item.Material) : []);
    } else {
      setAvailableMaterials([]);
    }
  };
    ...
    <SubModuleBar moduleData={moduleBarData} />
      ...
      <div className="form-container">
        <div className="form-section">
          <select value={selectedProjectId} onChange={handleProjectChange} className="dropdown4">
            ...
            <div key={index} className="material-row">
              <input 
                ...
                onChange={(e) => {
                  ...
                  const existingMaterials = [...new Set(stock.map((item) => item.Material))];
                  updatedMaterials[index].filteredMaterials = existingMaterials.filter((mat) =>
          ...
          <button className="add-more-btn" onClick={() => setMaterials([...materials, { materialName: "", searchTerm: "", filteredMaterials: [] }])}>Add More</button>
          <button className="submit-btn4" onClick={() => {
            console.log("Project ID:", selectedProjectId);
            console.log("Supplier:", selectedSupplier);
            console.log("Materials:", materials.map((m) => m.materialName));
            setShowPopup(true);
            setTimeout(() => setShowPopup(false), 3000);
          }}>Submit</button>
          {showPopup && <div className="popup-message">✅ Material Added Successfully!</div>}
        </div>
